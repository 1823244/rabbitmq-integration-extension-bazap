...
Функция ПоМэппингу(GUID, ВнешняяСистема) Экспорт
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.ксп_МэппингСправочникВидыНоменклатуры КАК Рег
		|ГДЕ
		|	Рег.GUID = &GUID
...
Процедура ДобавитьГУИД(GUID, Наименование, ВнешняяСистема) Экспорт
	...
	НЗ = РегистрыСведений.ксп_МэппингСправочникВидыНоменклатуры.СоздатьНаборЗаписей();
...
//	Параметр1 	- Тип1 - 
//
Процедура ДобавитьЗапись(GUID, Наименование, ВнешняяСистема, ВидНоменклатурыСсылка) Экспорт
	...
	стрк.Наименование = Наименование;  
	стрк.ВнешняяСистема = ВнешняяСистема;	
	стрк.ВидНоменклатуры = ВидНоменклатурыСсылка;	
	НЗ.Записать();
