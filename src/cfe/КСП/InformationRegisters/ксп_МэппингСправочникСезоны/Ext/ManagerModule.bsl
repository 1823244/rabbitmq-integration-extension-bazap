...
Функция ПоМэппингу(GUID, ВнешняяСистема) Экспорт
	...
	Запрос.Текст = 
		...
		|	Рег.Сезон КАК Сезон
		|ИЗ
		|	РегистрСведений.ксп_МэппингСправочникСезоны КАК Рег
		|ГДЕ
		|	Рег.GUID = &GUID
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат ВыборкаДетальныеЗаписи.Сезон;
	КонецЦикла;
...
Процедура ДобавитьГУИД(GUID, Наименование, ВнешняяСистема) Экспорт
	...
	НЗ = РегистрыСведений.ксп_МэппингСправочникСезоны.СоздатьНаборЗаписей();
