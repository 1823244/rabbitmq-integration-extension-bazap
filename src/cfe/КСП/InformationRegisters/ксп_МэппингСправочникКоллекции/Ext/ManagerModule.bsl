...
Функция ПоМэппингу(GUID, ВнешняяСистема) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Рег.Коллекция КАК Коллекция
		|ИЗ
		|	РегистрСведений.ксп_МэппингСправочникКоллекции КАК Рег
		|ГДЕ
		|	Рег.GUID = &GUID
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат ВыборкаДетальныеЗаписи.Коллекция;
	КонецЦикла;
...
Процедура ДобавитьГУИД(GUID, Наименование, ВнешняяСистема) Экспорт
	...
	НЗ = РегистрыСведений.ксп_МэппингСправочникКоллекции.СоздатьНаборЗаписей();
