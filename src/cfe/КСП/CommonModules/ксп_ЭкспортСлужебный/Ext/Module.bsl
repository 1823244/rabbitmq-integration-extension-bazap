...
Функция СоздатьУзелIdentification_Справочника(СсылкаНаОбъект) Экспорт
	...
	ИначеЕсли type = "Справочник.СтраныМира" Тогда
...
Функция ВыгрузитьОбъектПоСсылке(Обк) Экспорт
	...
	СтруктураОбъекта.Вставить("source", "BAZAP");
...
Процедура ВыполнитьЭкспорт_ГуидовНоменклатуры(НеНайденнаяНоменклатураМассив) Экспорт
	...
	Для сч = 0 По НеНайденнаяНоменклатураМассив.Количество()-1 Цикл
		...
		Если сч > 0 И сч % 20 = 0 Тогда
			...
			Исключение
			    т=ОписаниеОшибки();
			    ЗаписьЖурналаРегистрации("ИмпортИзЕРП", УровеньЖурналаРегистрации.Предупреждение,,,
			        "Ошибка экспорта ненайденной номенклатуры из Базы показов в УПП. Подробности: "+т);
			КонецПопытки;
...
Процедура ВыполнитьЭкспорт_ГуидовКонтрагентов(мНеНайденныеКонтрагентыМассив) Экспорт
	...
	Исключение
	    т=ОписаниеОшибки();
	    ЗаписьЖурналаРегистрации("ИмпортИзЕРП", УровеньЖурналаРегистрации.Предупреждение,,,
	        "Ошибка экспорта ненайденных контрагентов из Базы показов в УПП. Подробности: "+т);
	КонецПопытки;
